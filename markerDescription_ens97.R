...
markerDescription <- function(markers, verbose = F) {
  ...
  queryMax <- 399
  startIndex <- 1
  highestIndex <- length(genelist$entrezgene_id)
  ...
  while (TRUE) {
    ...
    genes.batch <- entrez_summary(db = "gene", id = genelist$entrezgene_id[startIndex:stopIndex], always_return_list = T)
  ...
  markers$entrez <- with(genelist, entrezgene_id[match(markers$entrez, hgnc_symbol)])
